...
import { Modal, ModalOverlay, ModalContent, ModalHeader, ModalCloseButton, ModalBody, ModalFooter, Text, Button, Tag, TagLabel, Box, useToast } from '@chakra-ui/react';
    ...
    return (
        <Modal onClose={() => setIsBuying(false)} isOpen={isBuying} isCentered>
            ...
            <ModalContent>
                ...
                <ModalCloseButton />
                <ModalBody>
                    <Text>Total tiles {selectedTiles.length}/1000 tiles</Text>
                    <MdMoney /> <Text>Total price {selectedTiles.length * 0.1} CET</Text>
                    <Box maxHeight="calc(100vh - 600px)" overflowX="hidden" overflowY="scroll" p="1" border="1px" rounded="lg">
                        {selectedTiles.map((tile) => (
                            <Tag size="sm" variant='outline' key={tile.id} colorScheme='blue'>
                    ...
                    <Text>{estateInfor?.lng},{estateInfor?.lat}</Text>
                </ModalBody>
                <ModalFooter>
